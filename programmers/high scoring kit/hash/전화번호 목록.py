...
# class Node :
#     def __init__(self, value, isNumberEnd):
#         self.children : dict[int, Node] = dict()
#         self.value : int = value
#         self.isNumberEnd : bool = isNumberEnd

# def solution(phone_book : list[str]) -> bool:
#     heads : dict[int, Node] = dict()
#     for num in phone_book: 
#         next : Node = None

#         for i, c in enumerate(num) :
#             options = heads if next is None else next.children
#             if c in options.keys() :
#                 if options[c].isNumberEnd or i == len(num) - 1 and options[c].children :
#                     return False
#                 next = options[c]
#             else :
#                 options[c] = Node(c, i == len(num) - 1)
#                 next = options[c]
